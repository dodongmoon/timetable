...
<body>
    ...
    <section class="toolbar">
      <div class="toolbar-main">
        <div>
          ...
          <p class="toolbar-meta">총 {student_count}명 · 생성 시각 {html.escape(timestamp)} · 검색 후 보이는 카드를 이미지처럼 한 장씩 인쇄합니다.</p>
        </div>
  ...
  <script>
    ...
    function openPrintPreview(popup, images) {{
      const sheets = images.map((image, index) => `
        <section class="sheet${{index < images.length - 1 ? " has-break" : ""}}">
...
<body>${{sheets}}</body>
    ...
    async function printVisibleCards() {{
      ...
      if (!popup) {{
        alert("인쇄 창이 차단되었습니다. 브라우저의 팝업 차단을 잠시 해제해 주세요.");
        return;
      }}

      popup.document.write("<!doctype html><html lang='ko'><head><meta charset='utf-8'><title>시간표 준비 중</title></head><body style='font-family: Apple SD Gothic Neo, Noto Sans KR, sans-serif; padding: 24px;'>시간표 이미지를 준비 중입니다...</body></html>");
      popup.document.close();

      const previousText = printButton.textContent;
      printButton.disabled = true;
      printButton.textContent = "이미지 준비 중...";
      ...
      try {{
        ...
        for (const card of visibleCards) {{
          const name = card.querySelector(".student-name")?.textContent?.trim() || "시간표";
        ...
        openPrintPreview(popup, images);
      }} catch (error) {{
        console.error(error);
        popup.close();
        alert("카드를 이미지로 준비하지 못했습니다. 새로고침 후 다시 시도해 주세요.");
      }} finally {{
        printButton.disabled = false;
        printButton.textContent = previousText;
      }}
    }}
